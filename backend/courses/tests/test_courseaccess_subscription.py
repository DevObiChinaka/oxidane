...
class TestCourseAccessManagement:
    ...
    def test_revoke_access(self, user, course):
        ...
        assert diff < 2  # Allow for 1 second in the past to ensure is_active=False
    ...
    def test_renew_from_subscription_no_expiration(self, user, course, plan):
        """Test renewing subscription sets expiration from user subscription"""
        ...
        access.renew_from_subscription()
        
        assert access.access_expires_at is not None
        # Should match user's subscription end date
        assert access.access_expires_at == user.subscription_end_date

    def test_renew_from_subscription_future_expiration(self, user, course, plan):
        """Test renewing updates to user subscription end date"""
        future = timezone.now() + timedelta(days=5)
        user.current_plan = plan
        user.subscription_end_date = timezone.now() + timedelta(days=35)
        user.save()
        ...
        # Should update to match user's subscription end date
        assert access.access_expires_at == user.subscription_end_date
...
class TestCourseAccessHelpers:
    ...
    def test_grant_subscription_access_creates(self, user, course, plan):
        ...
        assert access.access_granted_by == 'subscription'
        assert access.subscription_plan == plan
        # Should use user's subscription_end_date
        assert access.access_expires_at == user.subscription_end_date
